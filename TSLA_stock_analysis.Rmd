...
summary(seas_m)
plot(seas_m$coefficients, xlab="Week", ylab="Co-effecient Value",main="Seasonality Models Weights", type="o",lwd=2)
...
"""
The interpretation of this coefficient plot heavily suggests a pattern in stock price growth; a face-value analysis shows us that prices growth during the first half of each year (weeks 0-26) and then drop abruptly in the second half of the year. A continuation of this plot, however, into the next year, would suggest that growth occurs between ~week 30 of year i and week 30 of year i+1. Overall, the plot suggests a clear seasonal component where prices rise over ~60 weeks before falling again.
...
```{r results='hold',message=FALSE}
full_m<-tslm(ltsla_ts~ t + t2 + t3 + season + 0)
...
"""
An observation of the fitted values compared to their residuals demonstrates a periodic trend, roughly equivalent to a sine wave, and that the residuals still contain structure. This suggests that the nonlinear model combined with the seasonal component has not entirely eliminated the stochastic trend, or the presence of cycles might still be influencing the residuals.

##\n\n

The overall adjusted R-squared is 0.9952. This indicates that a high percentage of variation in Tesla stock prices are explained by our model. All the coefficients are statistically significant at the 95% confidence level. The F-statistic also allows us to reject the null that the overall model is not significant.
