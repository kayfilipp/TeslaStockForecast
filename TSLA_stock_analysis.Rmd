...
```{r results='hold',message=FALSE}
#cut data
fmltsla_ts=window(ltsla_ts,2010,c(2018,10))
frltsla_ts=window(ltsla_ts,c(2018,11),c(2019,19))
fmt=t[1:400]
fmt2=t2[1:400]
fmt3=t3[1:400]
...
```{r results='hold',message=FALSE}
plot(predict(full_m$fitted.values, h=61) ,shadecols="oldstyle", main="TSLA Stock Full Forecast", xlab="Year", ylab="Log-Adjusted Closing Price (in $)", lwd=2, col=1, ylim=c(2.75, 6))
lines(ltsla_ts,lwd=2, col=2)
legend('bottomright', legend=c( "Model+Forecast","Actual Value"), col=1:2, lwd=2)
```
